...
- (void)setStatus:(StatusModel *)status {
    ...
    if (status.share_count <= 0) {
        ...
        [self.share setTitle:[NSString stringWithFormat:@"%d", status.share_count] forState:UIControlStateNormal];
    }
    
    if (status.comment_conut <= 0) {
        [self.comment setTitle:@"评论" forState:UIControlStateNormal];
    } else {
        [self.comment setTitle:[NSString stringWithFormat:@"%d", status.comment_conut] forState:UIControlStateNormal];
    }
    
    if (status.like_count <= 0) {
        [self.like setTitle:@"点赞" forState:UIControlStateNormal];
    } else {
        [self.like setTitle:[NSString stringWithFormat:@"%d", status.like_count] forState:UIControlStateNormal];
    }
